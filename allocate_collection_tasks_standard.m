function [updated_robots, allocation, collection_paths] = allocate_collection_tasks_standard(robots, resource_list, grid_map, weights)
...
%   collection_paths   - struct array with path data and robot mapping
    ...
    fprintf('  Allocating %d resources to %d collector robots using Standard A* algorithm...\n', ...
    ...
    while any(~assigned_resources)
        ...
        % Calculate cost matrix using Standard A* for this round
        cost_matrix = calculate_astar_cost_matrix_standard(robot_positions, resource_list, ...
        ...
        for assignment = 1:num_assignments_this_round
            ...
            % Calculate actual path for this assignment using Standard A*
            robot_pos = robot_positions(actual_robot_id, :);
            resource_pos = resource_list(actual_resource_id, :);
            
            try
                % *** 关键修改：调用标准A*算法 ***
                path_result = astar_pathfinding_standard(robot_pos, resource_pos, grid_map, ...
    ...
    fprintf('  === Collection Task Allocation Summary (Standard A*) ===\n');
...
%% Calculate Standard A* cost matrix for available robots and resources
function cost_matrix = calculate_astar_cost_matrix_standard(robot_positions, resource_list, ...
                                                           available_robots, available_resources, ...
                                                           grid_map, robots, weights)
% Calculate cost matrix using Standard A* path planning (ignoring terrain)
    ...
    fprintf('    Calculating Standard A* cost matrix (%dx%d)...\n', ...
    ...
    fprintf('    Standard A* cost matrix calculated (finite costs only):\n');
    for i = 1:num_available_robots
        robot_id = available_robots(i);
