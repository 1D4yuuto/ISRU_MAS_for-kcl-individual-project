function grid_map = generateGridMap(map, block_size, obstacle_threshold, energy_threshold)
% Generate improved grid map with coverage-based rules
% Input:
...
%   grid_map - 200x200 grid map for A* algorithm
    ...
    for i = 1:grid_H
        for j = 1:grid_W
            ...
            energy_1_5_ratio = sum(block(:) == 3) / total_pixels;
            energy_2_ratio = sum(block(:) == 4) / total_pixels;
            ...
            elseif resource_ratio > 0
                ...
                grid_map(i,j) = 9;
            elseif energy_2_ratio > energy_threshold
                % If 4x energy terrain coverage > 40%, mark as 4x energy
                grid_map(i,j) = 4;
            elseif energy_1_5_ratio > energy_threshold
