function [updated_robots, transport_allocation, transport_paths] = allocate_transport_tasks(robots, resource_list, collection_results, base_pos, grid_map, weights)
% Allocate transport tasks to transporter robots based on collection completion
% Input:
...
%   transport_paths     - struct array with path data and robot mapping
    ...
    fprintf('  Allocating %d transport tasks to %d transporter robots...\n', ...
    ...
    while any(~assigned_resources)
        ...
        % Calculate cost matrix for transport tasks (round trip costs)
        cost_matrix = calculate_transport_cost_matrix(robot_positions, resource_list, ...
        ...
        for assignment = 1:num_assignments_this_round
            ...
            try
                ...
                outbound_path = astar_pathfinding(current_pos, resource_pos, grid_map, ...
                                                robots(robot_idx), weights);
                
                % Plan return path: resource -> base
                return_path = astar_pathfinding(resource_pos, base_pos, grid_map, ...
                                              robots(robot_idx), weights);
    ...
    fprintf('  === Transport Task Allocation Summary ===\n');
...
%% Calculate transport cost matrix (round-trip costs)
function cost_matrix = calculate_transport_cost_matrix(robot_positions, resource_list, ...
                                                      available_resources, base_pos, ...
                                                      grid_map, robots, weights)
% Calculate round-trip transport costs for all robot-resource combinations
    ...
    fprintf('    Calculating transport cost matrix (%dx%d)...\n', ...
    ...
    for i = 1:num_robots
        ...
        for j = 1:num_available_resources
            ...
            try
                ...
                outbound_path = astar_pathfinding(robot_pos, resource_pos, grid_map, ...
                                                robots(i), weights);
    ...
    if ~isempty(finite_costs)
        fprintf('    Transport costs: min=%.2f, max=%.2f, avg=%.2f\n', ...
